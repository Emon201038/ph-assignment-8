...
function generateTripDates(durationDays: number) {
...
function getRandomGuides(excludeId?: string): string[] {
  const guideIds = [
...
function getTripIncludesForTour(category: string): string[] {
...
export function generateTripsData(toursData: any[]) {
  const trips: any[] = [];
  const tripIncludes: any[] = [];
  ...
  for (const tour of toursData) {
    ...
    for (let i = 0; i < tripsPerTour; i++) {
      ...
      const priceAdjustment =
        Math.random() > 0.5 ? priceVariation : -priceVariation;
...
export { getRandomGuides, generateTripDates, getTripIncludesForTour };
